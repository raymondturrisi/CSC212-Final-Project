...
void KDTree<t>::nearest_neighbor_oftype_r(Node<t>* node, t &data, std::string wanted_type, t &result, unsigned int depth_r, double &best_dist) {
    ...
    if(node->struct_of_data.coords[i] < data.coords[i]) {
        ...
        nearest_neighbor_oftype_r(node->right_child, data, wanted_type, result, depth_r+1, best_dist);
        ...
        nearest_neighbor_oftype_r(node->left_child, data, wanted_type, result, depth_r+1, best_dist);
    }
