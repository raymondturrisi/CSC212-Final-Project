...
class DefaultLocation : public KDTree_Key {
    ...
    public:
        ...
        bool operator==(const DefaultLocation &dl) {
            if( (dl.type == this->type) && (dl.name == this->name) && (dl.yelp_url == this->yelp_url) && (dl.stars == this->stars) && (dl.street_address == this->street_address) && (dl.city == this->city) && (dl.state == this->state) && (dl.zip_code == this->zip_code) && (dl.phone_number == this->phone_number) && (dl.review_count == this->review_count)) {
                    return true;
                } else {
                    return false;
                }
