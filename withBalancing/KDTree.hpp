...
//Required Libraries
//#include "Node.hpp"
#include "Median.hpp"//Node.hpp is include in Median.hpp
#include <fstream>
#include <string>
#include <unistd.h>
#include <cmath>
...
class KDTree {
    public:
       ...
       //insert
        Node<t>* insert_balanced_r(std::vector<t*>& input_list, Node<t>* node, int depth_r); /*
        {
            Called by KDTree<t>(std::vector<t> vector_of_structs, unsigned int _dimensions) and iterates recursively tracking the depth, obtaining the discriminator from the number of dimensions.
...
template <typename t>
KDTree<t>::KDTree(std::vector<t*> vector_of_structs, unsigned int _dimensions)
: findMedian(MedianOfMedians<t>(vector_of_structs,_dimensions))
{
    this->dimensions = _dimensions;
    int depth = 0;
    this->head = insert_balanced_r(vector_of_structs, this->head, depth);
}
...
//insert recursively, called from within, builds a balanced tree
template <typename t>
Node<t>* KDTree<t>::insert_balanced_r(std::vector<t*>& input_list, Node<t>* node, int depth_r)
{
    ...
    {
        nodes++;
        return new Node<t>(input_list[0]);
    }
    ...
    findMedian.reset(input_list, i);

    //Places median value in node
    //Reorders the list so all values less than median are placed to the left of the median and all values greater than to the right
    int start = 0;
    node = new Node<t> (findMedian.select(start,(inputSize-1),medianIdx));
    nodes++;
    ...
    std::vector<t*> leftHalf(input_list.begin(), input_list.begin()+medianIdx);
    std::vector<t*> rightHalf(input_list.begin()+medianIdx+1, input_list.end());

    if (leftHalf.size() > 0)
    {
        node->left_child = this->insert_balanced_r(leftHalf, node->left_child, depth_r+1);//Recursively operates on left half of vector
    }
    if (rightHalf.size() > 0)
    {
        node->right_child = this->insert_balanced_r(rightHalf, node->right_child, depth_r+1);//Recursively operates on right half of vector
    }
    return node;
}
